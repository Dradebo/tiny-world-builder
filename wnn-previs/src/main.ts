...
practical.position.set(-2.4, 2.2, -1.9);
scene.add(practical);

const root = new THREE.Group();
root.name = 'wnn-shopfront';
scene.add(root);
...
function actorProxy(name: string, x: number, z: number, color: string) {
  ...
  root.add(group);
  register(group, 'actor-proxy');
  return group;
}

function chair(name: string, x: number, z: number, color: string) {
  const g = new THREE.Group();
  g.name = name;
  const seat = new THREE.Mesh(new THREE.BoxGeometry(.55,.08,.55), mat(color)); seat.position.y=.48;
  const back = new THREE.Mesh(new THREE.BoxGeometry(.55,.65,.08), mat(color)); back.position.set(0,.8,.24);
  g.add(seat,back);
  for (const sx of [-.22,.22]) for (const sz of [-.22,.22]) {
    const leg = new THREE.Mesh(new THREE.BoxGeometry(.06,.48,.06), mat(color)); leg.position.set(sx,.24,sz); g.add(leg);
  }
  g.position.set(x,0,z); root.add(g); register(g,'chair'); return g;
}
...
function buildRoom() {
  ...
  for (let i=0;i<12;i++) {
    const b=box(`bottle-${i+1}`,[.11,.38,.11],[-2.85+(i%6)*.42,1.38+Math.floor(i/6)*.55,-2.62],i%3===0?'#2e4c3a':'#5c3827');
  ...
  chair('wood-chair-a',-1.25,.65,'#8a5a31');
  chair('wood-chair-b',1.25,.65,'#8a5a31');
  chair('odd-chair',.2,1.75,'#467f45');
  ...
  actorProxy('host-proxy',-1.05,.2,'#665443');
  actorProxy('coanchor-proxy',1.05,.2,'#635041');
  actorProxy('witness-proxy',2.25,-1.05,'#4d4b45');
...
function select(obj: THREE.Object3D | null) {
  selected = obj;
  if (!obj) { transform.detach(); selectionLabel.textContent='Nothing selected'; return; }
...
function setCamera(id: string, animate = true) {
  ...
  activeCamera=preset[0]; shotName.textContent=activeCamera;
...
for (const [id] of CAMERA_PRESETS) {
  const b=document.createElement('button'); b.textContent=id; b.dataset.camera=id;
  if(id==='MASTER') b.classList.add('active');
  b.addEventListener('click',()=>{
    document.querySelectorAll('[data-camera]').forEach(x=>x.classList.remove('active')); b.classList.add('active'); setCamera(id);
...
document.querySelectorAll<HTMLButtonElement>('[data-light]').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('[data-light]').forEach(x=>x.classList.remove('active')); b.classList.add('active'); setLighting(b.dataset.light || 'NORMAL');
}));

function setAspect(value:'16:9'|'9:16'){
  activeAspect=value; frameGuide.classList.toggle('landscape',value==='16:9'); frameGuide.classList.toggle('portrait',value==='9:16');
...
document.querySelectorAll<HTMLButtonElement>('[data-transform]').forEach(button=>button.addEventListener('click',()=>{
  ...
  document.querySelectorAll('[data-transform]').forEach(b=>b.classList.remove('active')); button.classList.add('active');
}));
...
modelInput.addEventListener('change',async()=>{
  ...
  try{const gltf=await loader.loadAsync(url);const obj=gltf.scene;obj.name=file.name;register(obj,file.name);obj.position.set(0,0,2.1);obj.traverse((child:any)=>{if(child.isMesh){child.castShadow=true;child.receiveShadow=true;}});root.add(obj);select(obj);}finally{URL.revokeObjectURL(url);modelInput.value='';}
});
...
function tick(){if(transition){const t=Math.min(1,(performance.now()-transition.start)/transition.duration);const e=t<.5?2*t*t:1-Math.pow(-2*t+2,2)/2;camera.position.lerpVectors(transition.fromPos,transition.toPos,e);orbit.target.lerpVectors(transition.fromTarget,transition.toTarget,e);if(t>=1)transition=null;}orbit.update();renderer.render(scene,camera);requestAnimationFrame(tick);}tick();
