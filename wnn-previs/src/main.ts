...
import { clearAmbientCharacters, createCharacter, makeTableLife, triggerEditorialBeat, updateAmbientLife } from './ambientLife';
...
const ambient = new THREE.HemisphereLight('#f0d7b1', '#33261c', 1.15); scene.add(ambient);
const key = new THREE.DirectionalLight('#ffdca0', 2.0); key.position.set(5, 7, 6); key.castShadow = true; scene.add(key);
const practical = new THREE.PointLight('#ffb75f', 1.1, 9); practical.position.set(-2.4, 2.2, -1.9); scene.add(practical);
...
function box(name: string, size: [number,number,number], pos: [number,number,number], color: string, selectableFlag = false) {
  const mesh = new THREE.Mesh(new THREE.BoxGeometry(...size), mat(color)); mesh.name = name; mesh.position.set(...pos); mesh.castShadow = true; mesh.receiveShadow = true; root.add(mesh);
  if (selectableFlag) register(mesh, name); return mesh;
}
function register(obj: THREE.Object3D, assetId: string) { obj.userData.kind = 'prop'; obj.userData.assetId = assetId; obj.userData.instanceId = `${assetId}-${++objectCounter}`; selectable.add(obj); }
function slender(name: string, size: [number,number,number], pos: [number,number,number], color: string, parent: THREE.Group) {
  const m = new THREE.Mesh(new THREE.BoxGeometry(...size), mat(color, .72)); m.name=name; m.position.set(...pos); m.castShadow = true; m.receiveShadow = true; parent.add(m); return m;
}
function foldingChair(name: string, x: number, z: number, rotY = 0) {
  const g = new THREE.Group(); g.name = name; const wood = '#9a6433';
...
function foldingTable() {
  const g = new THREE.Group(); g.name = 'folding-table'; const wood = '#966032';
  ...
  root.add(g); register(g,'folding-table'); makeTableLife(root); return g;
}

function buildRoom() {
  root.clear(); selectable.clear(); clearAmbientCharacters(); transform.detach(); selected = null; objectCounter = 0; selectionLabel.textContent = 'Nothing selected';
  ...
  box('shop-back',[6.8,3.2,.12],[0,1.6,-3.0],'#896e53'); box('shop-left',[.12,3.2,4.0],[-3.4,1.6,-1.0],'#8b7056'); box('shop-right',[.12,3.2,4.0],[3.4,1.6,-1.0],'#8b7056'); box('threshold',[7.0,.18,2.0],[0,.09,1.0],'#7b7065');
  ...
  foldingTable();
  foldingChair('wood-chair-a',-1.22,.78,.08); foldingChair('wood-chair-b',1.24,.72,-.10); foldingChair('wood-chair-c',.18,1.85,Math.PI);
  for (let i=0;i<5;i++) box(`stacked-plastic-${i}`,[.58,.08,.58],[2.35,.25+i*.1,1.65],'#3e8a49');
  box('beer-crate',[.65,.48,.55],[-2.45,.24,1.8],'#76502e',true); box('broom-handle',[.05,1.7,.05],[2.7,.86,1.25],'#496b86');
  const broomHead=box('broom-head',[.65,.08,.16],[2.72,.08,1.25],'#3d6d42'); broomHead.rotation.y=.2; box('bucket',[.42,.48,.42],[2.0,.24,2.0],'#3b7850');
...
function select(obj: THREE.Object3D | null) { selected = obj; if (!obj) { transform.detach(); selectionLabel.textContent='Nothing selected'; return; } transform.attach(obj); selectionLabel.textContent=`${obj.name || obj.userData.assetId}`; }
...
function setCamera(id: string, animate = true) {
  ...
  activeCamera=preset[0]; shotName.textContent=activeCamera; camera.fov=fov; camera.updateProjectionMatrix(); const toPos=new THREE.Vector3(...pos); const toTarget=new THREE.Vector3(...target);
...
for (const [id] of CAMERA_PRESETS) {
  ...
  b.addEventListener('click',()=>{ document.querySelectorAll('[data-camera]').forEach(x=>x.classList.remove('active')); b.classList.add('active'); setCamera(id); if(id==='VERTICAL') setAspect('9:16'); if(id==='WITNESS'||id==='HUNT') triggerEditorialBeat(.9); }); cameraButtons.append(b);
}
function setLighting(id: string) {
  ...
  else if(id==='WITNESS_SHOCK'){renderer.toneMappingExposure=1.05;ambient.intensity=.55;key.intensity=2.4;practical.intensity=.65;triggerEditorialBeat(1.15);}
